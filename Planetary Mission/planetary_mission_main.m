...
for i = 1:6
    subplot(3,2,i);
    if i <= 2
    plot(tspan/24/3600,(kep_rsw_pert(:,i)-kep_car(:,i))/test(i),'Color','b','DisplayName','Moon+J2'); 
    % legend('Show');
    ...
    elseif i == 3 || i == 5
    plot(tspan/24/3600,rad2deg((kep_rsw_pert(:,i)-kep_car(:,i)))/test(i),'Color','b','DisplayName','Moon+J2');
        xlim([0,N_days]);
    % legend('Show');
    grid on
    title(titles{i},'Interpreter','latex','FontSize',15);
    ...
    elseif i==4
       temp = [zeros(1,1); rad2deg((kep_rsw_pert(2:end,i)-(kep_car(2:end,i)-2*pi)))/test(i)]; 
           plot(tspan/24/3600,temp,'Color','b','DisplayName','Moon+J2');
    ...
    elseif i==6
     temp = [zeros(1,1); (kep_rsw_pert(2:end,i)-unwrap(kep_car(2:end,i)))./kep_rsw_pert(2:end,i)];
      plot(tspan/24/3600,temp,'Color','b','DisplayName','Moon+J2'); %unwrap data before filtering
    % legend('Show');
    grid on
      xlim([0,N_days]);
    title(titles{i},'Interpreter','latex','FontSize',15);
...
grid on;
hold on;
plot3(car_pert(1,1),car_pert(1,2),car_pert(1,3),'bo','MarkerSize',12,'LineWidth',5);
plot3(car_pert(end,1),car_pert(end,2),car_pert(end,3),'ro','MarkerSize',12,'LineWidth',5);
h1 = gca;
earth_sphere(h1,'km');
legend('','','','','','','','Starting Point', 'Arrival Point','Interpreter','latex','FontSize',15)
title('Orbit(s) representation','FontSize',15,'Interpreter','latex') 
...
for i = 1:6
    subplot(3,2,i);
    if i <= 2
    plot(tspan/24/3600,kep_rsw_pert(:,i),'Color','r','DisplayName','Unfiltered');
        xlim([0,N_days]);
    legend('show');
    hold on
    ax = gca;
    ax.ColorOrderIndex = 5;
    plot(tspan/24/3600,movmean(kep_rsw_pert(:,i),high_filter(i)),'LineWidth',1.1,'DisplayName','Long term behaviour');
        xlim([0,N_days]);
    legend('show');
    plot(tspan/24/3600,movmean(kep_rsw_pert(:,i),low_filter(i)),'Color','b','DisplayName','Secular');
        xlim([0,N_days]);
    legend('show');
    grid on
    title(titles{i},'Interpreter','latex','FontSize',15);
    ...
    %legend;
    elseif i >= 3
    plot(tspan/24/3600,rad2deg(kep_rsw_pert(:,i)),'Color','r','DisplayName','Unfiltered');
        xlim([0,N_days]);
    legend('show');
    hold on
    ax = gca;
    ax.ColorOrderIndex = 5;
    plot(tspan/24/3600,rad2deg(movmean(kep_rsw_pert(:,i),high_filter(i))),'LineWidth',1.1,'DisplayName','Long term behaviour');
        xlim([0,N_days]);
    legend('show');
    plot(tspan/24/3600,rad2deg(movmean(kep_rsw_pert(:,i),low_filter(i))),'Color','b','DisplayName','Secular');
        xlim([0,N_days]);
    legend('show');
    grid on
    title(titles{i},'Interpreter','latex','FontSize',15);
...
for i = 1:6
    figure(length(findobj('type','figure'))+1);
    if i <= 2
    plot(tspan_tle,kep_tle(:,i),'Color','b','DisplayName','Numerical');
    hold on
    plot(tspan_tle,movmean(kep_tle(:,i),n_filter),'c','DisplayName','Filtered', 'LineWidth',1.5);
    plot(data(:,1),data(:,i+1),'Color','r','DisplayName','TLE', 'LineWidth',1.5);
    grid on
    title(titles{i});
    ...
    xlim([t0_tle,tf_tle]);
    elseif i >= 3
    plot(tspan_tle,rad2deg(kep_tle(:,i)),'Color','b','LineWidth',1.5,'DisplayName','Numerical');
    hold on
    grid on
    plot(tspan_tle,rad2deg(movmean(kep_tle(:,i),n_filter)),'c--','LineWidth',1.5,'DisplayName','Filtered');
    plot(data(:,1),data(:,i+1),'Color','r','LineWidth',1.5,'DisplayName','TLE');
    title(titles{i});
    xlabel('Time [MJD2000]','Interpreter','latex');
    ylabel(ylabels{i},'Interpreter','latex');
    legend;
